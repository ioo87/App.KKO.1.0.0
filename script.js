...
========================================================= */
/* =========================================================
   USAGE BADGES — shows how many times each tool was used
   (pulled from the existing history in localStorage).
   Uses textContent throughout; never touches innerHTML.
========================================================= */
const TOOL_TYPE_MAP = {
  mp3: 'trackMp3', pdf: 'trackPdf', qr: 'trackQr',
  compress: 'trackCompress', format: 'trackFormat', merge: 'trackMerge',
  watermark: 'trackWatermark', translate: 'trackTranslate', reader: 'trackReader'
};
...
function renderUsageBadges() {
  ...
  Object.entries(TOOL_TYPE_MAP).forEach(([type, cardId]) => {
    const card = document.getElementById(cardId);
    if (!card) return;

    // Remove any existing badge so we don't duplicate on re-render
    const old = card.querySelector('.usage-badge');
    if (old) old.remove();
    ...
    if (count && count > 0) {
      ...
      badge.textContent = String(count);  // textContent — user-safe number
      badge.setAttribute('aria-label', `ใช้ไปแล้ว ${count} ครั้ง`);
...
/* =========================================================
   NEON TAP RIPPLE — an expanding green circle that bursts
   from the exact tap/click position inside each tool card.
========================================================= */
function createNeonRipple(event, card) {
  const ripple = document.createElement('span');
  ripple.className = 'neon-ripple';

  const rect = card.getBoundingClientRect();
  const size = Math.max(rect.width, rect.height);
  // clientX/Y for mouse; fall back to card centre for keyboard
...
/* =========================================================
   HAPTIC FEEDBACK — a short vibration on supported devices
   (phones that have navigator.vibrate, e.g. most Android).
========================================================= */
function triggerHaptic() {
  if (navigator.vibrate) navigator.vibrate(8);
}

/* Wire ripple + haptic to every tool card (set up once at page load) */
document.querySelectorAll('.tool-icon-card').forEach(card => {
  card.addEventListener('click', (e) => {
    createNeonRipple(e, card);
    triggerHaptic();
  });
  card.addEventListener('touchstart', (e) => {
    if (e.touches.length > 0) {
      const t = e.touches[0];
      createNeonRipple({ clientX: t.clientX, clientY: t.clientY }, card);
      triggerHaptic();
    }
  }, { passive: true });
});
...
function enterApp() {
  ...
  // app-bar avatar (separate element)
  document.getElementById('menuAvatarDisplay').textContent = initial;
  document.getElementById('menuUserName').textContent = currentUser.name;
  showScreen('screen-menu');
}

/* =========================================================
   DRAWER — slide-in panel containing history, countdown,
   quote, share & sign-out. Opened by the ☰ button.
========================================================= */
...
document.getElementById('menuHamburgerBtn').addEventListener('click', openDrawer);
document.getElementById('drawerCloseBtn').addEventListener('click', closeDrawer);
document.getElementById('drawerBackdrop').addEventListener('click', closeDrawer);
// avatar in app bar also opens the drawer (mirrors common app pattern)
document.getElementById('menuAvatarBtn').addEventListener('click', openDrawer);
